package com.ep.linkedlist.chat.view;
...
import com.ep.linkedlist.chat.adapter.CircleTransform;
...
public class FullScreenImageActivity extends AppCompatActivity {
    ...
    private void setValues(){
        ...
        if (StringUtils.isNotBlank(urlPhotoUser)) {
            Glide.with(this).load(urlPhotoUser).centerCrop().transform(new CircleTransform(this)).override(40,40).into(ivUser);
        }
        else {
            ivUser.setImageResource(R.drawable.profile_default_image);
        }

        if (StringUtils.isNotBlank(urlPhotoClick)) {
